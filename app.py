...
# PDF deps (add reportlab to requirements.txt)
...
def pdf_watermark_and_footer(c: canvas.Canvas, doc):
    ...
    c.setFillColor(colors.HexColor("#E6EAF0"))  # very light gray
    c.translate(300, 400)
    c.rotate(30)
...
def build_pdf_report(
    ...
    styles.add(ParagraphStyle(name="H2", fontSize=13, leading=16, spaceBefore=8, spaceAfter=6, textColor=colors.HexColor("#0F172A")))
    ...
    # Insights note (charts not embedded)
    story.append(Paragraph("Insights (Charts shown in app)", styles["H2"]))
    story.append(Paragraph(
        "Charts are interactive in the web app. This PDF includes the numeric summaries and recommended actions.",
        styles["Body"]
    ))
    story.append(Spacer(1, 0.12*inch))
...
if opps_file and roles_file:
    ...
    label_with_tooltip(
        "Estimated Days Saved per Added Contact",
        "((Avg days Lost − Avg days Won) ÷ (Avg contacts Won − Avg contacts Lost)). Directional sensitivity."
    )
    show_value(f"{days_saved_per_extra_contact:.0f} days" if days_saved_per_extra_contact else "0 days")

    label_with_tooltip(
        "Estimated Reduction in Open Opp Age (if Open hits 2.0 contacts)",
        "Days saved per added contact × (2.0 − current open avg contacts)."
    )
    show_value(f"{est_days_saved_open:.0f} days" if est_days_saved_open else "0 days")
    ...
    # Insights charts (unchanged, interactive in app)
